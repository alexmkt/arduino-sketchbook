...
void loop() {
  ...
  if (pattern==0){
    ...
    randomness();
     ...
     myservo.write(analogRead(0)/5.69);              // tell servo to go to position in variable 'pos' 
...
//left to right
  ...
  while (offset<(cols)){
    ...
    for (i=0; i < strip.numPixels(); i++) {
        if ( (i%(rows*2)<rows && i%rows==offset) || (i%(rows*2)>=rows && ((rows-1)-(i%rows))==offset)) {
          strip.setPixelColor(i, Color(RedPart(Wheel(analogRead(0)/4) )/(255-audiobright),GreenPart(Wheel(analogRead(0)/4) )/(255-audiobright),BluePart(Wheel(analogRead(0)/4) )/(255-audiobright) )); 
        }else{
          strip.setPixelColor(i, 0); 
        }
